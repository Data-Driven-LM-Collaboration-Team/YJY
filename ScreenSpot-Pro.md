...
数据集构成：视频画面 + 最终结果图 + 关键帧的操作（低层级+高层级）
采集过程：以公开的 DeepCAD 数据集为基础，筛选出一部分CAD 模型作为数据生成的原始标准。然后将每个 CAD 模型的建模步骤和参数转换为自动执行的的鼠标、键盘操作指令，然后再通过自动化脚本执行操作指令，同步录制操作全过程的界面视频，录制的时候还要记录每一步操作的时间、类型与参数。最后对比生成的CAD模型和原始样本，把不合格的样本筛掉，将合格的整理为标准化数据，最终得到有效的数据集。

# 数据集设计
数据集构成：每条样本数据可以有如下内容，再补充关键帧操作
![star](./resource/GUI/star.png)

采集过程：导入文件，然后录屏与自动化采集脚本同时运行，脚本监听键盘鼠标事件，同样对应操作分为低层级和高层级标注，最终验收结果
